  ...
  async execute(orderId: string, dto: UpdateOrderStatusDto) {
    ...
    if (dto.message && dto.message.trim() !== '') {
      ...
      if (dto.status === 'APPROVED') {
        ...
        clientMessage = `Your order #${orderId.slice(-6)} status has been updated to ${dto.status}.`;
      }
    }
    ...
    try {
      ...
      if (!targetUserId) {
        console.error(
          `[Notification Skipped]: Could not find user id in order object`,
        );
      } else {
        await this.notificationService.createNotification(
          targetUserId,
        ...
        console.log(
          `[Notification DB Saved]: Custom message sent for User ${targetUserId}`,
        );
      }
    } catch (notiError: any) {
      console.error('⚠️ Failed to save notification to DB:', notiError.message);
